...
func main() {
	...
	err = server.AddHook(new(RedisHook), &HookOption{
...
func (h *RedisHook) Init(config any) error {
	h.Log.Info("Initialised")
	if _, ok := config.(*HookOption); !ok && config != nil {
		return mqtt.ErrInvalidConfigType
	}
	h.config = config.(*HookOption)
	if h.config.db == nil {
		return mqtt.ErrInvalidConfigType
	}
	go func() string {
		...
		for range time.Tick(5 * time.Second) {
			err := h.config.db.Set(context.Background(), "test1", "1", 10 * time.Second)
			if err != nil {
				return "error to ping to db"
			}
...
func (h *RedisHook) Provides(b byte) bool {
	return true
}
